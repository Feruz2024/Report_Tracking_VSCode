...
function CompletedCampaignList({ refresh }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([campaignsData, clientsData, stationsData, monitoringPeriodsData]) => {
        setCampaigns(Array.isArray(campaignsData) ? campaignsData.filter(c => c.status === "COMPLETED") : []);
  ...
  if (loading) return <div>Loading completed campaigns...</div>;
  ...
  return (
    <>
      ...
      <div>
        <h2 style={{ textAlign: 'center', marginBottom: 24 }}>Completed Campaigns</h2>
        <div style={{
          display: 'flex',
          flexWrap: 'wrap',
          ...
          {filteredCampaigns.length === 0 && <div style={{ color: '#888', fontStyle: 'italic' }}>No completed campaigns match the filter.</div>}
          {filteredCampaigns.map((campaign, idx) => {
            ...
            const pastelColors = ['#eaf6ff', '#eafff3', '#f6eaff', '#fffbe6', '#eafcff', '#fff2eb'];
